...
class fcfs {
	public static void main(String[] args) throws NumberErrorException {
		...
		if (numProcess <= 0) throw new NumberErrorException();
		...
		for (int i = 0; i < numProcess; i++) {
			...
			if (arrivalTime[i] < 0) throw new NumberErrorException();
			System.out.print("Enter Burst time of P" + (i+1) + ": ");
			burstTime[i] = mys.nextInt();
			if (burstTime[i] < 0) throw new NumberErrorException();
		}
		
		int completionTime[] = new int[numProcess];
		for (int i = 0; i<numProcess; i++) completionTime[i] = 0;
		for (int i = 0; i < numProcess; i++) {
			for (int j = 0; j <= i; j++) {
		...
		for (int i = 0; i < numProcess; i++) {
			System.out.println(" P" + (1+i) + "\t\t" + arrivalTime[i] + "ms" + "\t" + burstTime[i] + "ms" + "\t" + completionTime[i] + "ms" + "\t" + turnAround[i] + "ms" + "\t" + waitingTime[i] +"ms");
		}
